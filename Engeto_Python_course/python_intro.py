...
# Krok 1: Vytvoření prázdného seznamu pro uložení vyčištěných slov
# **Seznam** je v Pythonu proměnlivá (mutable) a uspořádaná kolekce, která umožňuje duplikáty.
# Používáme seznam, protože potřebujeme zachovat pořadí slov při jejich zpracování.
# Seznamy jsou efektivní pro přidávání položek a iterace.
clear_words = list()  # `list()` vytvoří prázdný seznam. Ekvivalentní zápisu `[]`.

# Krok 2: Zpracování každého slova v textu
# `text.split()` rozdělí text na jednotlivá slova podle mezer (výchozí oddělovač).
# Vrátí seznam slov, přičemž zachová jejich pořadí v původním textu.
for word in text.split():
    # Odstranění interpunkce a převod slov na malá písmena
    # `strip()` odstraní zadané znaky (zde `,.:;`) z počátku a konce řetězce.
    # Převod na malá písmena zajistí, že se slova jako "Slovo" a "slovo" považují za stejná.
    clear_words.append(
        word.strip(",.:;").lower()
    )
    # `.append()` přidá nový prvek na konec seznamu. Modifikuje seznam na místě.

# Krok 3: Vytvoření prázdného slovníku pro počítání slov
# **Slovník** (nebo `dict`) je neuspořádaná, proměnlivá kolekce klíč-hodnota v Pythonu.
# Používáme slovník, protože umožňuje rychlé vyhledávání a aktualizace každého slova (klíč) a jeho počtu (hodnota).
# Klíče musí být jedinečné, takže každé slovo se ve slovníku může objevit jen jednou.
words_count = dict()  # `dict()` inicializuje prázdný slovník. Ekvivalentní zápisu `{}`.

# Krok 4: Počítání frekvence jednotlivých slov
# Iterace přes seznam vyčištěných slov.
for word in clear_words:
    # Zkontrolujte, zda slovo již není ve slovníku:
    # - Pokud tam není, přidejte jej jako klíč a nastavte jeho hodnotu na 1 (první výskyt).
    # - Pokud tam je, zvýšíte jeho počet o 1.
    # Operátor `not in` kontroluje, zda klíč ve slovníku chybí.
    if word not in words_count:
        words_count[word] = 1  # Přidání nového slova s počtem 1.
    else:
        words_count[word] += 1  # Zvýšení počtu stávajícího slova.

# Krok 5: Identifikace tří nejvyšších frekvencí
# `words_count.values()` získá všechny hodnoty (frekvence) ze slovníku.
# `list(...)` převede hodnoty na seznam, aby je bylo možné seřadit.
# `sorted(..., reverse=True)` seřadí seznam sestupně (od nejvyšší po nejnižší).
# `[:3]` ořízne první tři prvky ze seřazeného seznamu, čímž získáte tři nejvyšší frekvence.
three_most_common_words = sorted(
    list(words_count.values()), reverse=True
)[:3]

# Krok 6: Vytiskněte celý slovník frekvencí slov
# Tisk slovníku ukazuje všechna slova a jejich frekvence.
print(words_count)

# Krok 7: Nalezení čtyř nejčastějších slov
# `sorted()` seřadí klíče slovníku podle jejich odpovídajících hodnot.
# Parametr `key=words_count.get` říká `sorted()`, aby použil počet slov jako klíč pro třídění.
# `reverse=True` zajistí sestupné pořadí, takže slova s vyšším počtem se zobrazí jako první.
# `[:4]` ořízne první čtyři slova s nejvyššími frekvencemi.
print(sorted(words_count, key=words_count.get, reverse=True)[:4], "více než 3")

# Krok 8: Vytvoření seznamu pro uložení slov s třemi nejčastějšími frekvencemi
# Používáme seznam, protože potřebujeme uložit několik dvojic (frekvence, slovo) a zachovat jejich pořadí.
# **Trojice** (tuples) jsou v Pythonu neměnné (immutable), uspořádané kolekce, ideální pro ukládání párovaných dat.
vysledky = list()

# Krok 9: Identifikace slov, která odpovídají třem nejvyšším frekvencím
# Iterace přes jednotlivé klíče (slova) ve slovníku:
for count in words_count:
    # Zkontrolujte, zda frekvence slova patří mezi tři nejčastější frekvence.
    if words_count[count] in three_most_common_words:
        # Přidejte do seznamu dvojici (frekvence, slovo).
        vysledky.append((words_count[count], count))

# Krok 10: Seřazení a zobrazení výsledků podle frekvence sestupně
# `sorted()` se používá k seřazení seznamu dvojic podle jejich prvního prvku (frekvence).
# `reverse=True` zajistí sestupné pořadí.
# `enumerate()` přidá index (začínající od 1) ke každé položce v seznamu, což je užitečné pro řazení.
for index, tupl in enumerate(sorted(vysledky, reverse=True), 1):
    # Použijeme formátovaný řetězec (f-string) pro výstup. F-stringy umožňují vkládání proměnných přímo do řetězců.
    print(f"{index}. {tupl[0]} - {tupl[1]}", sep="\n")
